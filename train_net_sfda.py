...
from layers import CrossEntropyLabelSmooth, TripletLoss, SoftTripletLoss, TripletLossXBM, MMS_loss, multiModalMarginLossUnsupervised,Unsupervised_prototype_multiModalMarginLoss,SoftEntropy
...
def median_heuristic(x, y):
    ...
    # 获取所有非零距离的中位数
    all_distances = torch.cat([x_dist.flatten(), y_dist.flatten(), xy_dist.flatten()])
    bandwidth = torch.median(all_distances)
...
if __name__ == '__main__':
    ...
    logger = setup_logger("TransReID", output_dir, cfg, if_train=True)
    ...
    if cfg.load_param:
        if cfg.MODEL.BASE == 7:
            model.load_param_neo(cfg.VIT.init)
        elif cfg.MODEL.BASE == 10:
            model.load_param_neo(cfg.CLIP.init)
        else:
    ...
    model_ema = make_model(cfg, num_class=num_classes, camera_num=camera_num, view_num=view_num)
    if cfg.load_param:
        if cfg.MODEL.BASE == 7:
            model_ema.load_param_neo(cfg.VIT.init)
        elif cfg.MODEL.BASE == 10:
            model_ema.load_param_neo(cfg.CLIP.init)
        else:
            model_ema.load_param_neo(cfg.MMT.init_1)
    ...
    losses_soft_ce = AverageMeter()
    losses_soft_tri = AverageMeter()
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % 4 == 1:
            ...
            dict_f, dict_f1, dict_f2, dict_f3, _ = extract_features_neo(model_ema, target_train_loader, print_freq=50)
            ...
            if (epoch == 1 and (cfg.TARGET_DATASETS.NAMES == 'RGBNT100' )) or cfg.TARGET_DATASETS.NAMES == 'RGBNT201' or cfg.TARGET_DATASETS.NAMES == 'market_to_RGBNT201':
                ...
                model_ema.module.classifier.weight.data[source_classes:source_classes+target_classes].copy_(F.normalize(target_centers, dim=1).float().cuda())
                model_ema.module.classifier_R.weight.data[source_classes:source_classes+target_classes].copy_(F.normalize(target_centers_R, dim=1).float().cuda())
                model_ema.module.classifier_N.weight.data[source_classes:source_classes+target_classes].copy_(F.normalize( target_centers_N, dim=1).float().cuda())
                model_ema.module.classifier_T.weight.data[source_classes:source_classes+target_classes].copy_(F.normalize(target_centers_T, dim=1).float().cuda())
            ...
            model_ema.module.prototypes_tar_ori = reliable_feats_list[0]
            model_ema.module.prototypes_tar_R = reliable_feats_list[1]
            model_ema.module.prototypes_tar_N = reliable_feats_list[2]    
            model_ema.module.prototypes_tar_T = reliable_feats_list[3]   
        ...
        criterion_op = MMS_loss().cuda()
        criterion_3m = Unsupervised_prototype_multiModalMarginLoss(margin=0.5,dist_type=dist_type).cuda()
        criterion_3m_p = multiModalMarginLossUnsupervised(margin=0.5,dist_type='l2').cuda()
        criterion_ce_soft = SoftEntropy().cuda()
        criterion_tri_soft = SoftTripletLoss(margin=None).cuda()
        ...
        losses_tri.reset()
        losses_xbm.reset()
        losses_soft_ce.reset()
        losses_soft_tri.reset()
        losses_cls.reset()
        losees_3m.reset()
        losses_mmd.reset()
        ...
        if cfg.dataloader_iter:     
            ...
            for n_iter in range(len(train_loader_target)):
                ...
                with amp.autocast(enabled=True):
                    ...
                    target_ori_score_ema, target_ori_ema, target_RGB_score_ema, target_RGB_global_ema, target_NI_score_ema, target_NI_global_ema, target_TI_score_ema, target_TI_global_ema = model_ema(img_target, label=target_target, cam_label=target_cam_target, view_label=target_view_target, domain=1)
                    ...
                    #INTRA-MODALITY MEAN TEACHER ######################################################
                    loss_ce_soft = criterion_ce_soft(target_ori_score, target_ori_score_ema) + criterion_ce_soft(target_RGB_score, target_RGB_score_ema) + criterion_ce_soft(target_NI_score, target_NI_score_ema) + criterion_ce_soft(target_TI_score, target_TI_score_ema) 
                    loss_tri_soft = criterion_tri_soft(target_ori, target_ori_ema, target_target) + criterion_tri_soft(target_RGB_global, target_RGB_global_ema, target_target) + criterion_tri_soft(target_NI_global, target_NI_global_ema, target_target) + criterion_tri_soft(target_TI_global, target_TI_global_ema, target_target) 
                    ######################################################
                    ...
                    # MMD loss _______________________________________________________________________________________________________________________________________________
                    ...
                    loss = loss_ce + loss_tri + loss_xbm + loss_ce_soft + loss_tri_soft + loss_mmd + loss_3m
                scaler.scale(loss).backward()
                scaler.step(optimizer)
                scaler.update()

                update_ema_variables(model, model_ema, cfg.SECRET.ALPHA, epoch*len(train_loader_target) + n_iter)
                ...
                losses_soft_ce.update(loss_ce_soft.item())
                losses_soft_tri.update(loss_tri_soft.item())
                ...
                if (n_iter + 1) % log_period == 0:
                    ...
                    logger.info("Epoch[{}] Iteration[{}/{}] Loss: {:.3f}, loss_ce: {:.3f}, loss_tri: {:.3f}, loss_xbm: {:.3f}, losses_mmd: {:.3f}, losses_mmm: {:.3f}, losses_soft_ce: {:.3f}, losses_soft_tri: {:.3f}, Acc_s_rgb: {:.3f},  Acc_s_ni: {:.3f}, Acc_s_ti: {:.3f}, Acc_t_rgb: {:.3f}, Acc_t_ni: {:.3f}, Acc_t_ti: {:.3f}, Base Lr: {:.2e}"
                                .format(epoch, (n_iter + 1), len(train_loader),
                                        loss_meter.avg, losses_ce.avg, losses_tri.avg, losses_xbm.avg, losses_mmd.avg, losees_3m.avg, losses_soft_ce.avg, losses_soft_tri.avg, acc_r_source_meter.avg, acc_n_source_meter.avg, acc_t_source_meter.avg, acc_r_target_meter.avg, acc_n_target_meter.avg, acc_t_target_meter.avg, scheduler._get_lr(epoch)[0]))
            scheduler.step(epoch)
            ...
            # 确定较大的数据集
            source_len = len(train_loader.dataset)
            target_len = len(train_loader_target.dataset)
            max_len = max(len(train_loader), len(train_loader_target))
            ...
            for n_iter in range(max_len):
                ...
                # 自适应选择源域数据
                try:
                    img_source, vid_source, target_cam_source, target_view_source, _ = next(source_iter)
                ...
                # 自适应选择目标域数据
                try:
                    img_target, vid_target, target_cam_target, target_view_target, _ = next(target_iter)
                except StopIteration:
                    target_iter = iter(train_loader_target)
                    img_target, vid_target, target_cam_target, target_view_target, _ = next(target_iter)
                ...
                # 确保源域和目标域数据在同一批次中平衡
                source_batch_size = img_source['RGB'].size(0)
                target_batch_size = img_target['RGB'].size(0)

                if source_batch_size != target_batch_size:
                    if source_batch_size < target_batch_size:
                        # 源域样本少，使用 RandomIdentitySampler 进行采样
                        sampler = RandomIdentitySampler(train_loader.dataset, target_batch_size, num_instances=4)
                        sampled_indices = list(iter(sampler))
                        img_source = {key: value[sampled_indices] for key, value in img_source.items()}
                        target_source = target_source[sampled_indices]
                        target_cam_source = target_cam_source[sampled_indices] if target_cam_source is not None else None
                        target_view_source = target_view_source[sampled_indices] if target_view_source is not None else None
                    else:
                        # 目标域样本少，使用 RandomIdentitySampler 进行采样
                        sampler = RandomIdentitySampler(train_loader_target.dataset, source_batch_size, num_instances=4)
                        sampled_indices = list(iter(sampler))
                        img_target = {key: value[sampled_indices] for key, value in img_target.items()}
                        target_target = target_target[sampled_indices]
                        target_cam_target = target_cam_target[sampled_indices] if target_cam_target is not None else None
                        target_view_target = target_view_target[sampled_indices] if target_view_target is not None else None
        ...
        logger.info("Epoch {} done. Time per batch: {:.3f}[s] Speed: {:.2f}[samples/s]"
                    .format(epoch, time_per_batch, cfg.SOLVER.IMS_PER_BATCH / time_per_batch))
        ...
        if epoch % eval_period == 0:
            ...
            for n_iter, (img, vid, camid, camids, target_view, _) in enumerate(val_loader):
                with torch.no_grad():
                    ...
                    feat,feat1,feat2,feat3 = model_ema(img)
            ...
            logger.info("model_ema validating on dataset:{}".format(cfg.DATASETS.NAMES))
            ...
            if mAP >= best_index1['mAP']:
                ...
                torch.save(model_ema.state_dict(),
                            os.path.join(output_dir, cfg.DATASETS.NAMES + '_{}_best_ema.pth'.format(cfg.SOLVER.SEED)))
            ...
            logger.info("model_ema validating on dataset:{}".format(cfg.TARGET_DATASETS.NAMES))
            logger.info("Validation Results - Epoch: {}".format(epoch))
            logger.info("mAP: {:.2%}".format(mAP))
            for r in [1, 5, 10]:
                logger.info("CMC curve, Rank-{:<3}:{:.2%}".format(r, cmc[r - 1]))
            if mAP >= best_index['mAP']:
                ...
                torch.save(model_ema.state_dict(),
                            os.path.join(output_dir, cfg.TARGET_DATASETS.NAMES + '_{}_best_ema.pth'.format(cfg.SOLVER.SEED)))
            ...
            logger.info("model test on source_dataset: {}".format(cfg.DATASETS.NAMES))
            do_inference_neo(cfg,
                        model,
                        val_loader,
                        num_query,
            ...
            logger.info("model test on target dataset: {}".format(cfg.TARGET_DATASETS.NAMES))
            do_inference(cfg,
                        model,
                        target_val_loader,
                        target_query,
