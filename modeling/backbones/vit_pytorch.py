...
class Trans(nn.Module):
    ...
    def load_param_neo(self, model_path):
        param_dict = torch.load(model_path, map_location='cpu')
        if 'model' in param_dict:
            param_dict = param_dict['model']
        if 'state_dict' in param_dict:
        ...
        for k, v in param_dict.items():
            ...
            try:
                ...
                # 如果参数名不匹配，则跳过加载该参数，并打印错误消息
                print(f"Skipping parameter '{k}' because it does not match the current model.")
            except:
                # 其他异常情况
                print('===========================ERROR=========================')
                print(f"shape do not match in k: {k}: param_dict{v.shape} vs self.state_dict(){self.state_dict()[k].shape}")
