...
import torchvision
import torch
from idm.models.idm_module import AttentionIDM
from collections import OrderedDict


__all__ = ['ResNet', 'resnet18', 'resnet34', 'resnet50', 'resnet101',
           'resnet152', 'resnet50_neo']
...
class ResNet_neo(nn.Module):
    ...
    def __init__(self, depth, pretrained=True, cut_at_pooling=False,
                 num_features=0, norm=False, dropout=0, num_classes=0):
        super(ResNet_neo, self).__init__()
        self.pretrained = pretrained
        self.depth = depth
        self.cut_at_pooling = cut_at_pooling
        self.norm = norm
        ...
        # Construct base (pretrained) resnet
        if depth not in ResNet_neo.__factory:
            raise KeyError("Unsupported depth:", depth)
        resnet = ResNet_neo.__factory[depth](pretrained=pretrained)
        resnet.layer4[0].conv2.stride = (1,1)
        resnet.layer4[0].downsample[0].stride = (1,1)
        self.base = nn.Sequential(
            resnet.conv1, resnet.bn1, resnet.maxpool, # no relu
        ...
        if not self.cut_at_pooling:
            ...
            # Append new layers (only feature transformation, no BN)
            if self.has_embedding:
                self.feat = nn.Linear(out_planes, self.num_features)
                init.kaiming_normal_(self.feat.weight, mode='fan_out')
                init.constant_(self.feat.bias, 0)
    ...
    def forward(self, x):
        ...
        return x  # Always return features without BN
    ...
    def reset_params(self):
        ...
        resnet = ResNet_neo.__factory[self.depth](pretrained=self.pretrained)
        self.base[0].load_state_dict(resnet.conv1.state_dict())
        self.base[1].load_state_dict(resnet.bn1.state_dict())
        self.base[2].load_state_dict(resnet.maxpool.state_dict())
        self.base[3].load_state_dict(resnet.layer1.state_dict())
...
def resnet50_neo(**kwargs):
    return ResNet_neo(50, **kwargs)