...
class RGBNT201(BaseImageDataset):
    ...
    def __init__(self, root='', verbose=True, **kwargs):
        ...
        c_trains = self.camera_datasets(self.train_dir, relabel=True)
    ...
    def camera_datasets(self, dir_path, relabel=False):
        ...
        for img_path_RGB in img_paths_RGB:
            ...
            pid = int(jpg_name.split('_')[0][0:6])
            camid = int(jpg_name.split('_')[1][3])  # 提取相机ID
            trackid = -1
            camid -= 1  # 将相机ID调整为从0开始

            if relabel:
                pid = pid2label[pid]

            # 根据相机ID将样本添加到对应子数据集
            dataset_c[camid].append((img, pid, camid, trackid))
