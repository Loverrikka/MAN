...
def extract_multipart_vit_features(model, data_loader, num_parts):
    ...
    with torch.no_grad():
        for i, (imgs, pids, cams, _, fnames) in enumerate(tqdm.tqdm(data_loader)):
            
            imgs['RGB'] = to_torch(imgs['RGB']).cuda()
            imgs['NI'] = to_torch(imgs['NI']).cuda()
            imgs['TI'] = to_torch(imgs['TI']).cuda()
            # imgs = to_torch(imgs).cuda()
            cams = to_torch(cams).cuda()
            ...
            for fname, output, pid in zip(fnames, out_dict['global'], pids):
                key = fname if isinstance(fname, str) else fname[0]
                global_feats[key] = output
                labels[key] = pid
