...
class BridgeFeatLoss_IM(nn.Module):
    def __init__(self, mi_estimator):
        super(BridgeFeatLoss_IM, self).__init__()
        self.mi_estimator = mi_estimator

    def compute_bridge_loss_with_mi(self, feats_s, feats_t, feats_mixed, lam):
        # 计算欧氏距离
        dist_mixed2s = ((feats_mixed - feats_s) ** 2).sum(1, keepdim=True)
        dist_mixed2t = ((feats_mixed - feats_t) ** 2).sum(1, keepdim=True)
        ...
        # 计算互信息
        mi_s_mixed = self.mi_estimator(feats_s, feats_mixed)
        mi_t_mixed = self.mi_estimator(feats_t, feats_mixed)
        
        # 结合互信息到距离计算中
        dist_mixed2s = dist_mixed2s / (1 + mi_s_mixed)
        dist_mixed2t = dist_mixed2t / (1 + mi_t_mixed)

        dist_mixed = torch.cat((dist_mixed2s, dist_mixed2t), 1)
        lam_dist_mixed = (lam * dist_mixed).sum(1, keepdim=True)
        bridge_loss = lam_dist_mixed.mean()

        return bridge_loss

    def forward(self, feats_s, feats_t, feats_mixed, lam):
        return self.compute_bridge_loss_with_mi(feats_s, feats_t, feats_mixed, lam)
